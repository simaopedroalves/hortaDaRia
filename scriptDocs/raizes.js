...
async function fetchSheetsData() {
    const CACHE_KEY      = 'sheetsCache_raizes';
    const CACHE_DATE_KEY = 'sheetsCacheDate_raizes';
    ...
    if (cached && cachedDate && parseInt(cachedDate) >= getLastScheduledTime()) {
        console.log('A usar cache do Sheets (raizes)');
        return JSON.parse(cached);
    }

    try {
        console.log('A fazer fetch ao Sheets (raizes)...');
        ...
        console.error('Erro Sheets (raizes):', e);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const produtos   = object.raizes.map(item => {
    ...
    if (available.length > 0) {
        secRaizes.appendChild(createSubtitle('✅ Disponíveis', true));
        available.forEach(item => secRaizes.appendChild(createCard(item)));
    }

    if (unavailable.length > 0) {
        secRaizes.appendChild(createSubtitle('⏳ Brevemente disponíveis', false));
        unavailable.forEach(item => secRaizes.appendChild(createCard(item)));
    }
