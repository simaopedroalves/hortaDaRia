...
async function fetchSheetsData() {
    const CACHE_KEY      = 'sheetsCache_verduras';
    const CACHE_DATE_KEY = 'sheetsCacheDate_verduras';
    ...
    if (cached && cachedDate && parseInt(cachedDate) >= getLastScheduledTime()) {
        console.log('A usar cache do Sheets (verduras)');
        return JSON.parse(cached);
    }

    try {
        console.log('A fazer fetch ao Sheets (verduras)...');
        ...
        console.error('Erro Sheets (verduras):', e);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const produtos   = object.verduras.map(item => {
    ...
    if (available.length > 0) {
        secVerduras.appendChild(createSubtitle('✅ Disponíveis', true));
        available.forEach(item => secVerduras.appendChild(createCard(item)));
    }

    if (unavailable.length > 0) {
        secVerduras.appendChild(createSubtitle('⏳ Brevemente disponíveis', false));
        unavailable.forEach(item => secVerduras.appendChild(createCard(item)));
    }
