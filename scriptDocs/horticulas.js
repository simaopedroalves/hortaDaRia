...
async function fetchSheetsData() {
    const CACHE_KEY      = 'sheetsCache_horticulas';
    const CACHE_DATE_KEY = 'sheetsCacheDate_horticulas';
    ...
    if (cached && cachedDate && parseInt(cachedDate) >= getLastScheduledTime()) {
        console.log('A usar cache do Sheets (horticulas)');
        return JSON.parse(cached);
    }

    try {
        console.log('A fazer fetch ao Sheets (horticulas)...');
        ...
        console.error('Erro Sheets (horticulas):', e);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const produtos   = object.horticulas.map(item => {
    ...
    if (available.length > 0) {
        secHorticulas.appendChild(createSubtitle('✅ Disponíveis', true));
        available.forEach(item => secHorticulas.appendChild(createCard(item)));
    }

    if (unavailable.length > 0) {
        secHorticulas.appendChild(createSubtitle('⏳ Brevemente disponíveis', false));
        unavailable.forEach(item => secHorticulas.appendChild(createCard(item)));
    }
