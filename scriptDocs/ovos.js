...
async function fetchSheetsData() {
    const CACHE_KEY      = 'sheetsCache_ovos';
    const CACHE_DATE_KEY = 'sheetsCacheDate_ovos';
    ...
    if (cached && cachedDate && parseInt(cachedDate) >= getLastScheduledTime()) {
        console.log('A usar cache do Sheets (ovos)');
        return JSON.parse(cached);
    }

    try {
        console.log('A fazer fetch ao Sheets (ovos)...');
        ...
        console.error('Erro Sheets (ovos):', e);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const produtos   = object.ovos.map(item => {
