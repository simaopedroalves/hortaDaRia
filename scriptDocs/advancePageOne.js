...
function removeRequiredAttributeOnPostalCode() {
    let previousRequiredElements = document.querySelector('.cp');
    if (previousRequiredElements) {
...
function addRequiredAttribute() {
    ...
    previousRequiredElements.forEach(el => el.setAttribute('required', ''));
    if (floorInput) floorInput.removeAttribute('required');
}
...
// Validação do select #cp (Código Postal - parte 1)
if (firstCPNumber && firstCPNumber.tagName === 'SELECT') {
    firstCPNumber.addEventListener('change', () => {
        if (firstCPNumber.value && firstCPNumber.value !== '') {
...
function getCartItemsFromLocalStorage() {
    ...
    console.log('cart content:', cartContent.value);
}

function getTotalToPay() {
    if (!totalToPayInput) return;
    const finalPrice = JSON.parse(localStorage.getItem('finalPrice'));
    console.log('finalPrice:', finalPrice);
    totalToPayInput.value = finalPrice ? finalPrice + '€' : '0€';
}
...
// ─── DESATIVAR BOTÃO FORA DOS DIAS DE ENCOMENDA ───────────────────────────────
