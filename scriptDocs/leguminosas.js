...
async function fetchSheetsData() {
    const CACHE_KEY      = 'sheetsCache_leguminosas';
    const CACHE_DATE_KEY = 'sheetsCacheDate_leguminosas';
    ...
    if (cached && cachedDate && parseInt(cachedDate) >= getLastScheduledTime()) {
        console.log('A usar cache do Sheets (leguminosas)');
        return JSON.parse(cached);
    }

    try {
        console.log('A fazer fetch ao Sheets (leguminosas)...');
        ...
        console.error('Erro Sheets (leguminosas):', e);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const produtos   = object.leguminosas.map(item => {
    ...
    if (available.length > 0) {
        secLeguminosas.appendChild(createSubtitle('✅ Disponíveis', true));
        available.forEach(item => secLeguminosas.appendChild(createCard(item)));
    }

    if (unavailable.length > 0) {
        secLeguminosas.appendChild(createSubtitle('⏳ Brevemente disponíveis', false));
        unavailable.forEach(item => secLeguminosas.appendChild(createCard(item)));
    }
